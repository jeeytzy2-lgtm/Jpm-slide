let handler = async (m, { jeeybtz, text, isCreator, reply }) => {
  if (!isCreator) return reply("⚠️ Fitur ini hanya bisa digunakan oleh Developer bot!");

  const channels = await jeeybtz.newsletterFetchAllParticipating();
  ...
  if (m.chat.endsWith("@newsletter")) {
    ...
    try {
      if (action === "on") {
        await jeeybtz.newsletterMute(m.chat);
        reply(`Notifikasi Channel ini berhasil dimatikan.`);
      } else {
        await jeeybtz.newsletterUnmute(m.chat);
  ...
  for (const idx of indexes) {
    ...
    try {
      if (isOn) {
        await jeeybtz.newsletterMute(target.id);
        hasil.push(`🔕 ${target.name || target.id} dimute.`);
      } else if (isOff) {
        await jeeybtz.newsletterUnmute(target.id);
